# Kelvin Thuranira Resume
### Android Developer (4+ Years) | Flutter | AI & Offline-First Systems

[Email](mailto:kelvinthuranira591@gmail.com) • [Portfolio](https://kpotfolio.web.app) • [LinkedIn](https://www.linkedin.com/in/kelvin-thuranira-485844231) • [Download PDF Resume](https://www.mediafire.com/file/vo9zm1omxehdbir/KELVIN_RESUME_%25281%2529.pdf/file)

---

## About Me
Android developer with 4+ years of experience building scalable, high-performance mobile applications using Flutter and native Android technologies. I specialize in offline-first systems, AI-powered features, and production-ready applications that solve real-world problems.

---

## Core Skills
- Android Development (Java, Kotlin, XML)
- Flutter & Dart (Cross-platform Mobile & Web)
- Offline-First App Architecture & Caching
- Firebase & Firestore Integration
- REST API Integration
- Jetpack Components (Room, LiveData, ViewModel)
- On-Device AI (OCR, Image Processing)

---

## Other Skills
- Customer Service (Experience as Toll Attendant)
- Good Communication Skills (Experience as Teacher)
- Typing Speed: Fast & Accurate
- Networking Skills (HTTP, SNI, Proxies)
- Network Administration (Internal College Networks)
- Svelte Web Development
- Figma & UI/UX Design

---

## Work History
- **Freelance Developer** — 2024 to Present
- **Trainer – Tala Technical Institute** — 2022
- **Network Administrator Intern** — 2021

---

## Education
- **Diploma in Information Technology** — Meru University of Science and Technology, 2018  
  [Certificate](https://www.mediafire.com/file/sv3tcu2puxa9y5g/DIPcert.pdf/file)

---

## Achievements & Certifications
- GitHub Certificate — [View](https://www.mygreatlearning.com/certificate/JGASKNTK)  
- Linux Fundamentals — [View](https://www.mygreatlearning.com/certificate/AYCIRGTP)  
- Java Networking — [View](https://www.mygreatlearning.com/certificate/KSXSHFTZ)  
- Java Programming — [View](https://www.mygreatlearning.com/certificate/MCATWOEZ)  
- Typing Skills Certificate — [View](https://www.typing.com/api/v1/student/tests/184791776/129391219/certificate?language=en)  
- GitHub Version Control Certificate — [View](https://www.mediafire.com/file/cenwx95k42bukjv/GithubCert.pdf/file)

---

## Featured Projects

### Eye – AI Offline Image Scanner
Flutter Android app using TensorFlow Lite for offline image analysis, OCR, and similarity detection.  
[Documentation](https://github.com/realkevo/EyeReadme/tree/main?tab=readme-ov-file) • [Download APK](https://apkpure.com/eye-image-scanner/com.toptech.eye)

### SMAP – Smart Management App
Android (Java/XML) app for digitizing toll operations.

### Web Projects
- [Portfolio](https://kpotfolio.web.app)  
- [Mso Safaris](https://msosafaris.web.app)  
- [Mumg Cereals](https://mumgcereals.web.app/)

---

*Build systems that work offline, scale reliably, and solve real problems.*

---

## References
- GitHub Profile — [https://github.com/realkevo](https://github.com/realkevo)  
- Moja Expressway HR Contact — 0769 252297
